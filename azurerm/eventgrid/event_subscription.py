# Code generated by `ubx sdk gen` from azurerm_eventgrid_event_subscription. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as sdk

@dataclasses.dataclass
class EventSubscription_AdvancedFilter_BoolEquals:
    key: Any = None
    value: Any = None

@dataclasses.dataclass
class EventSubscription_AdvancedFilter_IsNotNull:
    key: Any = None

@dataclasses.dataclass
class EventSubscription_AdvancedFilter_NumberGreaterThan:
    key: Any = None
    value: Any = None

@dataclasses.dataclass
class EventSubscription_AdvancedFilter_NumberIn:
    key: Any = None
    values: Any = None

@dataclasses.dataclass
class EventSubscription_AdvancedFilter_NumberInRange:
    key: Any = None
    values: Any = None

@dataclasses.dataclass
class EventSubscription_AdvancedFilter_StringBeginsWith:
    key: Any = None
    values: Any = None

@dataclasses.dataclass
class EventSubscription_AdvancedFilter:
    bool_equals: Any = None
    is_not_null: Any = None
    is_null_or_undefined: Any = None
    number_greater_than: Any = None
    number_greater_than_or_equals: Any = None
    number_in: Any = None
    number_in_range: Any = None
    number_less_than: Any = None
    number_less_than_or_equals: Any = None
    number_not_in: Any = None
    number_not_in_range: Any = None
    string_begins_with: Any = None
    string_contains: Any = None
    string_ends_with: Any = None
    string_in: Any = None
    string_not_begins_with: Any = None
    string_not_contains: Any = None
    string_not_ends_with: Any = None
    string_not_in: Any = None

@dataclasses.dataclass
class EventSubscription_AzureFunctionEndpoint:
    function_id: Any = None
    max_events_per_batch: Any = None
    preferred_batch_size_in_kilobytes: Any = None

@dataclasses.dataclass
class EventSubscription_DeadLetterIdentity:
    type: Any = None
    user_assigned_identity: Any = None

@dataclasses.dataclass
class EventSubscription_DeliveryProperty:
    header_name: Any = None
    secret: Any = None
    source_field: Any = None
    type: Any = None
    value: Any = None

@dataclasses.dataclass
class EventSubscription_RetryPolicy:
    event_time_to_live: Any = None
    max_delivery_attempts: Any = None

@dataclasses.dataclass
class EventSubscription_StorageBlobDeadLetterDestination:
    storage_account_id: Any = None
    storage_blob_container_name: Any = None

@dataclasses.dataclass
class EventSubscription_StorageQueueEndpoint:
    queue_message_time_to_live_in_seconds: Any = None
    queue_name: Any = None
    storage_account_id: Any = None

@dataclasses.dataclass
class EventSubscription_SubjectFilter:
    case_sensitive: Any = None
    subject_begins_with: Any = None
    subject_ends_with: Any = None

@dataclasses.dataclass
class EventSubscription_Timeouts:
    create: Any = None
    delete: Any = None
    read: Any = None
    update: Any = None

@dataclasses.dataclass
class EventSubscription_WebhookEndpoint:
    active_directory_app_id_or_uri: Any = None
    active_directory_tenant_id: Any = None
    base_url: Any = None
    max_events_per_batch: Any = None
    preferred_batch_size_in_kilobytes: Any = None
    url: Any = None

_EventSubscription_AdvancedFilter_BoolEqualsFields = {
    "key": sdk.FieldSpec(wire_name="key"),
    "value": sdk.FieldSpec(wire_name="value"),
}

_EventSubscription_AdvancedFilter_IsNotNullFields = {
    "key": sdk.FieldSpec(wire_name="key"),
}

_EventSubscription_AdvancedFilter_NumberGreaterThanFields = {
    "key": sdk.FieldSpec(wire_name="key"),
    "value": sdk.FieldSpec(wire_name="value"),
}

_EventSubscription_AdvancedFilter_NumberInFields = {
    "key": sdk.FieldSpec(wire_name="key"),
    "values": sdk.FieldSpec(wire_name="values"),
}

_EventSubscription_AdvancedFilter_NumberInRangeFields = {
    "key": sdk.FieldSpec(wire_name="key"),
    "values": sdk.FieldSpec(wire_name="values"),
}

_EventSubscription_AdvancedFilter_StringBeginsWithFields = {
    "key": sdk.FieldSpec(wire_name="key"),
    "values": sdk.FieldSpec(wire_name="values"),
}

_EventSubscription_AdvancedFilterFields = {
    "bool_equals": sdk.FieldSpec(
        wire_name="bool_equals",
        kind="list",
        fields=_EventSubscription_AdvancedFilter_BoolEqualsFields,
    ),
    "is_not_null": sdk.FieldSpec(
        wire_name="is_not_null",
        kind="list",
        fields=_EventSubscription_AdvancedFilter_IsNotNullFields,
    ),
    "is_null_or_undefined": sdk.FieldSpec(
        wire_name="is_null_or_undefined",
        kind="list",
        fields=_EventSubscription_AdvancedFilter_IsNotNullFields,
    ),
    "number_greater_than": sdk.FieldSpec(
        wire_name="number_greater_than",
        kind="list",
        fields=_EventSubscription_AdvancedFilter_NumberGreaterThanFields,
    ),
    "number_greater_than_or_equals": sdk.FieldSpec(
        wire_name="number_greater_than_or_equals",
        kind="list",
        fields=_EventSubscription_AdvancedFilter_NumberGreaterThanFields,
    ),
    "number_in": sdk.FieldSpec(
        wire_name="number_in",
        kind="list",
        fields=_EventSubscription_AdvancedFilter_NumberInFields,
    ),
    "number_in_range": sdk.FieldSpec(
        wire_name="number_in_range",
        kind="list",
        fields=_EventSubscription_AdvancedFilter_NumberInRangeFields,
    ),
    "number_less_than": sdk.FieldSpec(
        wire_name="number_less_than",
        kind="list",
        fields=_EventSubscription_AdvancedFilter_NumberGreaterThanFields,
    ),
    "number_less_than_or_equals": sdk.FieldSpec(
        wire_name="number_less_than_or_equals",
        kind="list",
        fields=_EventSubscription_AdvancedFilter_NumberGreaterThanFields,
    ),
    "number_not_in": sdk.FieldSpec(
        wire_name="number_not_in",
        kind="list",
        fields=_EventSubscription_AdvancedFilter_NumberInFields,
    ),
    "number_not_in_range": sdk.FieldSpec(
        wire_name="number_not_in_range",
        kind="list",
        fields=_EventSubscription_AdvancedFilter_NumberInRangeFields,
    ),
    "string_begins_with": sdk.FieldSpec(
        wire_name="string_begins_with",
        kind="list",
        fields=_EventSubscription_AdvancedFilter_StringBeginsWithFields,
    ),
    "string_contains": sdk.FieldSpec(
        wire_name="string_contains",
        kind="list",
        fields=_EventSubscription_AdvancedFilter_StringBeginsWithFields,
    ),
    "string_ends_with": sdk.FieldSpec(
        wire_name="string_ends_with",
        kind="list",
        fields=_EventSubscription_AdvancedFilter_StringBeginsWithFields,
    ),
    "string_in": sdk.FieldSpec(
        wire_name="string_in",
        kind="list",
        fields=_EventSubscription_AdvancedFilter_StringBeginsWithFields,
    ),
    "string_not_begins_with": sdk.FieldSpec(
        wire_name="string_not_begins_with",
        kind="list",
        fields=_EventSubscription_AdvancedFilter_StringBeginsWithFields,
    ),
    "string_not_contains": sdk.FieldSpec(
        wire_name="string_not_contains",
        kind="list",
        fields=_EventSubscription_AdvancedFilter_StringBeginsWithFields,
    ),
    "string_not_ends_with": sdk.FieldSpec(
        wire_name="string_not_ends_with",
        kind="list",
        fields=_EventSubscription_AdvancedFilter_StringBeginsWithFields,
    ),
    "string_not_in": sdk.FieldSpec(
        wire_name="string_not_in",
        kind="list",
        fields=_EventSubscription_AdvancedFilter_StringBeginsWithFields,
    ),
}

_EventSubscription_AzureFunctionEndpointFields = {
    "function_id": sdk.FieldSpec(wire_name="function_id"),
    "max_events_per_batch": sdk.FieldSpec(wire_name="max_events_per_batch"),
    "preferred_batch_size_in_kilobytes": sdk.FieldSpec(wire_name="preferred_batch_size_in_kilobytes"),
}

_EventSubscription_DeadLetterIdentityFields = {
    "type": sdk.FieldSpec(wire_name="type"),
    "user_assigned_identity": sdk.FieldSpec(wire_name="user_assigned_identity"),
}

_EventSubscription_DeliveryPropertyFields = {
    "header_name": sdk.FieldSpec(wire_name="header_name"),
    "secret": sdk.FieldSpec(wire_name="secret"),
    "source_field": sdk.FieldSpec(wire_name="source_field"),
    "type": sdk.FieldSpec(wire_name="type"),
    "value": sdk.FieldSpec(wire_name="value"),
}

_EventSubscription_RetryPolicyFields = {
    "event_time_to_live": sdk.FieldSpec(wire_name="event_time_to_live"),
    "max_delivery_attempts": sdk.FieldSpec(wire_name="max_delivery_attempts"),
}

_EventSubscription_StorageBlobDeadLetterDestinationFields = {
    "storage_account_id": sdk.FieldSpec(wire_name="storage_account_id"),
    "storage_blob_container_name": sdk.FieldSpec(wire_name="storage_blob_container_name"),
}

_EventSubscription_StorageQueueEndpointFields = {
    "queue_message_time_to_live_in_seconds": sdk.FieldSpec(wire_name="queue_message_time_to_live_in_seconds"),
    "queue_name": sdk.FieldSpec(wire_name="queue_name"),
    "storage_account_id": sdk.FieldSpec(wire_name="storage_account_id"),
}

_EventSubscription_SubjectFilterFields = {
    "case_sensitive": sdk.FieldSpec(wire_name="case_sensitive"),
    "subject_begins_with": sdk.FieldSpec(wire_name="subject_begins_with"),
    "subject_ends_with": sdk.FieldSpec(wire_name="subject_ends_with"),
}

_EventSubscription_TimeoutsFields = {
    "create": sdk.FieldSpec(wire_name="create"),
    "delete": sdk.FieldSpec(wire_name="delete"),
    "read": sdk.FieldSpec(wire_name="read"),
    "update": sdk.FieldSpec(wire_name="update"),
}

_EventSubscription_WebhookEndpointFields = {
    "active_directory_app_id_or_uri": sdk.FieldSpec(wire_name="active_directory_app_id_or_uri"),
    "active_directory_tenant_id": sdk.FieldSpec(wire_name="active_directory_tenant_id"),
    "base_url": sdk.FieldSpec(wire_name="base_url"),
    "max_events_per_batch": sdk.FieldSpec(wire_name="max_events_per_batch"),
    "preferred_batch_size_in_kilobytes": sdk.FieldSpec(wire_name="preferred_batch_size_in_kilobytes"),
    "url": sdk.FieldSpec(wire_name="url"),
}

@dataclasses.dataclass
class EventSubscriptionConfig:
    advanced_filtering_on_arrays_enabled: Any = None
    event_delivery_schema: Any = None
    eventhub_id: Any = None
    expiration_time_utc: Any = None
    hybrid_connection_id: Any = None
    id: Any = None
    included_event_types: Any = None
    labels: Any = None
    name: Any = None
    scope: Any = None
    service_bus_queue_id: Any = None
    service_bus_topic_id: Any = None
    advanced_filter: Any = None
    azure_function_endpoint: Any = None
    dead_letter_identity: Any = None
    delivery_identity: Any = None
    delivery_property: Any = None
    retry_policy: Any = None
    storage_blob_dead_letter_destination: Any = None
    storage_queue_endpoint: Any = None
    subject_filter: Any = None
    timeouts: Any = None
    webhook_endpoint: Any = None

EventSubscription = sdk.ResourceBinding(
    wire_type="azurerm_eventgrid_event_subscription",
    fields={
        "advanced_filtering_on_arrays_enabled": sdk.FieldSpec(wire_name="advanced_filtering_on_arrays_enabled"),
        "event_delivery_schema": sdk.FieldSpec(wire_name="event_delivery_schema"),
        "eventhub_id": sdk.FieldSpec(wire_name="eventhub_id"),
        "expiration_time_utc": sdk.FieldSpec(wire_name="expiration_time_utc"),
        "hybrid_connection_id": sdk.FieldSpec(wire_name="hybrid_connection_id"),
        "id": sdk.FieldSpec(wire_name="id"),
        "included_event_types": sdk.FieldSpec(wire_name="included_event_types"),
        "labels": sdk.FieldSpec(wire_name="labels"),
        "name": sdk.FieldSpec(wire_name="name"),
        "scope": sdk.FieldSpec(wire_name="scope"),
        "service_bus_queue_id": sdk.FieldSpec(wire_name="service_bus_queue_id"),
        "service_bus_topic_id": sdk.FieldSpec(wire_name="service_bus_topic_id"),
        "advanced_filter": sdk.FieldSpec(
            wire_name="advanced_filter",
            kind="list",
            fields=_EventSubscription_AdvancedFilterFields,
        ),
        "azure_function_endpoint": sdk.FieldSpec(
            wire_name="azure_function_endpoint",
            kind="list",
            fields=_EventSubscription_AzureFunctionEndpointFields,
        ),
        "dead_letter_identity": sdk.FieldSpec(
            wire_name="dead_letter_identity",
            kind="list",
            fields=_EventSubscription_DeadLetterIdentityFields,
        ),
        "delivery_identity": sdk.FieldSpec(
            wire_name="delivery_identity",
            kind="list",
            fields=_EventSubscription_DeadLetterIdentityFields,
        ),
        "delivery_property": sdk.FieldSpec(
            wire_name="delivery_property",
            kind="list",
            fields=_EventSubscription_DeliveryPropertyFields,
        ),
        "retry_policy": sdk.FieldSpec(
            wire_name="retry_policy",
            kind="list",
            fields=_EventSubscription_RetryPolicyFields,
        ),
        "storage_blob_dead_letter_destination": sdk.FieldSpec(
            wire_name="storage_blob_dead_letter_destination",
            kind="list",
            fields=_EventSubscription_StorageBlobDeadLetterDestinationFields,
        ),
        "storage_queue_endpoint": sdk.FieldSpec(
            wire_name="storage_queue_endpoint",
            kind="list",
            fields=_EventSubscription_StorageQueueEndpointFields,
        ),
        "subject_filter": sdk.FieldSpec(
            wire_name="subject_filter",
            kind="list",
            fields=_EventSubscription_SubjectFilterFields,
        ),
        "timeouts": sdk.FieldSpec(
            wire_name="timeouts",
            kind="object",
            fields=_EventSubscription_TimeoutsFields,
        ),
        "webhook_endpoint": sdk.FieldSpec(
            wire_name="webhook_endpoint",
            kind="list",
            fields=_EventSubscription_WebhookEndpointFields,
        ),
    },
)
